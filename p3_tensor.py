#X와 Y의 상관관계를 분석하느 기초적인 선형 회귀 모델을 만들고 실행
...
#name : 나중에 텐서보드등으로 값의 변화를 추적하거나 살펴보기 쉽게 하기 위해 이름을 붙인다.
...
with tf.Session() as sess:
	...
	for step in range(100):
		...
		#이 때, 가설 수식에 넣어야할 실제값을 feed_dict를 통해 전달
		_, cost_val = sess.run([train_op, cost], feed_dict={X: x_data, Y:y_data})
		print(step, cost_val, sess.run(W), sess.run(b))

	#최적화가 완료된 모델에 테스트 값을 넣고 결과과 잘나오는지 확인
	print("\n=== test ===")
